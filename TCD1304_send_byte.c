...
#define SIGNAL_AVERAGE 100   // average the signal

#define START_MARKER 0xAA55  // Distinct pattern for frame start
#define PROTOCOL_VERSION 1   // For future compatibility
...
// Replace print_capture_buffer with this binary transfer function
void send_capture_buffer(uint16_t *buffer, int depth) {
    // Send header
    uint16_t header[4];
    header[0] = START_MARKER;        // Start marker
    header[1] = PROTOCOL_VERSION;    // Protocol version
    header[2] = depth;               // Number of samples
    header[3] = 0;                   // Reserved for future use
    
    // Send header
    fwrite(header, sizeof(uint16_t), 4, stdout);
    
    // Send data
    fwrite(buffer, sizeof(uint16_t), depth, stdout);
    
    // Calculate and send simple checksum (sum of all values)
    uint32_t checksum = 0;
    for (int i = 0; i < depth; i++) {
        checksum += buffer[i];
    }
    uint16_t checksum16 = checksum & 0xFFFF;
    fwrite(&checksum16, sizeof(uint16_t), 1, stdout);
...
int main()
{
    ...
    while (true)
    {
        ...
        printf("\n\rCapture finished\n\r");
        send_capture_buffer(capture_buf_sum, CAPTURE_DEPTH);
        sleep_ms(30);
    }
}